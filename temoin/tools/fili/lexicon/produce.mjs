...
const UNTRANSLATABLE = [
  { side: 'tool', name: '--pageR', what: "le rayon du cadre de la démonstration — c'est le rayon racine, et Fili ne l'expose pas comme jeton : chez lui le rayon racine EST la marge, il n'a pas de nom propre." },
  ...
  { side: 'tool', name: '--type-scale', what: "l'intervalle des titres. Ce n'est pas un jeton mais une décision d'entrée : elle vaut " + (geo.entries?.intervalHeadings ?? '—') + " chez Fili." },
  { side: 'tool', name: 'fontSize', what: "la taille du corps, envoyée à part. Chez Fili c'est le jeton type-corps." },
...
if (missing.length) {
  console.error(`\n  🔴 REFUS DE STATUER — jetons introuvables dans fili/geometry.json :\n     ${missing.join(', ')}\n`)
  process.exit(2)
}
...
const piece = {
  ...
  $rule: "Aucune valeur n'est déclarée ici. Chaque entrée pointe sur un jeton de fili/geometry.json, et la génération refuse de statuer si l'un manque.",
  ...
  $ruling: {
    ...
    ceAllows: "Dans une feuille de style écrite à la main, les noms de l'outil sont utilisables tels quels : src/lexicon.generated.css les fait pointer sur les jetons du dépôt.",
