...
| `/mouvement` · Mouvement | 🟢 | Fondation depuis le 7 septembre (sous Geste — arbitrage délégué). Trois preuves qui se lisent à l'œil (refonte du 8 septembre, après le tour des références) : la main invisible (d'abord la faute, puis comme il faut, six étapes sous-titrées, lentes), la course (le même menu quatre fois côte à côte, un seul « Ouvrir », le même instant, ralenti ×5 écrit à l'écran, le rang d'arrivée écrit quand la transition finit ; puis le même menu à trois vitesses — trop vite, juste, trop lent), dire ce qu'on voit (six tuiles au survol, le juste à côté du mot, les fautes exagérées jusqu'à être évidentes, ralenti ×3 écrit) ; quatre paires ; huit règles en liste, neuf lignes de code | **Verrouillée le 8 septembre : douze épreuves réécrites avec les preuves, trois courses vertes, course complète sur la machine d'Auteur — six pages vertes.** |
