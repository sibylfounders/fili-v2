...
export function Button({
  ...
  return (
    <button
      ...
      onClick={onPress}
      disabled={desactive}
      className={`inline-flex items-center justify-center rounded-controle border-systeme px-5 py-3 text-fin font-moyenne transition-colors duration-base ease-standard disabled:cursor-not-allowed ${ALLURE[variante]}`}
    >
      {children}
    </button>
  )
