...
| `/mouvement` · Mouvement | 🟢 | Fondation (sous Geste). Trois règles en trois comparaisons, le mauvais et le bon côte à côte, un seul bouton joue les deux (page reprise le 8 septembre sur les règles d'écriture d'Auteur : montrer, nommer, expliquer seulement si ça manque) — la trace (une carte de tâche passe de « À faire » à « Terminé » : elle glisse, ou elle disparaît et reparaît), la cause (le même menu de projet, depuis son bouton ou depuis nulle part), le regard (une donnée change sur un tableau de bord : une carte bouge, ou les huit) ; le répertoire : le moteur en neuf lignes lues au registre, six règles en liste | **Verrouillée le 8 septembre : onze épreuves écrites avec la page (dont l'écriture d'Auteur, mesurée), trois courses vertes, course complète sur la machine d'Auteur — six pages vertes.** |
