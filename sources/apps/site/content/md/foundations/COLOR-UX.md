...
layer: ux
type: foundation
version: 2.3.0 # 2.3.0 : LA MARQUE RESTE ENTIÈRE — SCISSION PRIMARY / PRIMARY-TEXT (retour d'Auteur, 2026-08-24 : « on ne retrouve pas la couleur de marque dans le primary »). L'aplat d'action (primary) porte désormais la couleur saisie TELLE QUELLE, quel que soit son éclat — son texte on-primary est cherché du côté qui tient (noir sur une marque claire, blanc sur une sombre). Seul primary-text — le lien, du texte posé sur les fonds — est calé AA quand la marque ne tient pas 4,5:1 : on assombrit le lien, jamais la marque. Nouveaux jetons : primary-text, primary-text-hover ; les consommateurs de texte (liens, kickers, badges, navigation active) basculent dessus. La gamme primitive 50–950 dérivée de la saisie s'affiche sur la page (illustration : un rôle ne consomme jamais une primitive). Huit primaires extrêmes testées : zéro faute sur les paires des deux thèmes. 2.2.0 : LA FAMILLE DEVIENT GÉNÉRÉE — UNE SEULE DÉCISION D'ENTRÉE : PRIMARY (décision d'Auteur, 2026-08-24). Le moteur du témoin (OKLCH, paires calculées par recherche de clarté) est porté au kit (kit/derivation.mjs) et CALIBRÉ SUR LA CHARTE : à la primaire de la charte (#4F46E5), il la reproduit (écart maximal mesuré : 8 crans 8-bit). Règles du calcul : neutres teintés à la teinte de primary à clartés de charte (C15) ; marque ancrée sur primary, textes on-* cherchés à leur seuil (C7) ; accent = l'écart de teinte de la charte, conservé (−55°) ; états aux ancres de charte, tirés d'un quart du déplacement de primary, bornés ±12° (C3 : un rouge reste un rouge) ; toute paire sous son seuil est recalée, jamais laissée en dessous (C7, C9). kit/app/tokens.css est désormais GÉNÉRÉ (node kit/derivation.mjs --css) — changer la famille, c'est changer primary et régénérer, jamais une valeur à la main. La page vivante dérive en direct et vérifie les 40 paires des deux thèmes. 2.1.0 : LA CHARTE FAIT AUTORITÉ SUR LES VALEURS (2026-08-24) — la charte de conception Fili, remise par l'Auteur et versée au dépôt (docs/charte/filicharte_6.html), devient la source des valeurs claires : background blanc dominant / surface grise (les rôles du kit étaient inversés), accent cyan #0891B2 dont l'unique métier est l'anneau de focus (il entoure, n'accueille jamais de texte), tons warning (#92400E) et info (#1D4ED8) entrés AVEC leur premier consommateur (la page En situation — C4/C5 tenues), surface-hover. Le thème sombre reste au registre du dépôt, la charte ne le couvrant pas encore — dit en C13. Pas de text-muted : le gris pâle de la charte (2,54:1 sur blanc) ne porte jamais un texte — sans consommateur, pas de jeton (C9). 2.0.0 : REPRISE AU MOULE V2 — le 4e paquet de la reprise des lois de l'étude entre au corpus (thread « la couleur entre au kit », 2026-08-23 ; paquet rédigé le 2026-08-13, claude/reprise-couleur-regles.md). Les 25 lois du fonds deviennent : un principe de tête (« par rôle, jamais par valeur » — verdict du ménage), seize règles C1–C16 (énoncé + mesure décidable + test + dépendance + verdict), trois mesures de rendu M1–M3 pour le futur instrument, trois pièces de référence. L'arbitrage d'Auteur du texte pâle (2026-08-13) ferme la contradiction S14 par le haut : la norme ne connaît pas d'exception « métadonnées », aucun jeton de texte ne descend sous le seuil (C9) — l'ex-R12 est renversée en le disant, la dette « liste fermée » du ménage est dissoute. La famille entre au kit avec ses valeurs résolues en clair ET en sombre (kit/app/tokens.css), gris pâle remonté d'emblée. Le CRITERE de la 1.4.0 est conservé à l'identique (porté par M1). Historique antérieur : voir 1.5.0 dans l'historique git.
