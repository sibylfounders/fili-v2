...
test('1 · la carte du zoom s’ouvre au ×2 : le corps affiché et le corps rendu sont ceux du moteur à la largeur réelle ; l’éteindre redescend au repos ; « vw seul » ne gagne pas un pixel', async () => {
  for (const W of WIDTHS) {
    ...
    const zoom = () => p.locator(`${card} .button`, { hasText: '×2' })
    const read = () => text(p, `${card} .mono.muted`, 0).then((t) => numbers(t.replace(/.*=/, ''))[0])
    const render = () => calcPx(p, `${card} [style*="font-size"]`, 'fontSize')
    assert.equal(await zoom().getAttribute('aria-pressed'), 'true', `${W} — le zoom est allumé d’entrée`)
    ok(await read(), rounded(body(W, 2)), `${W} — ×2 affiché`); ok(await render(), body(W, 2), `${W} — ×2 rendu`)
    ...
    ok(await read(), rounded(body(W)), `${W} — repos affiché`); ok(await render(), body(W), `${W} — repos rendu`)
    await zoom().click()
    ok(await render(), body(W, 2), `${W} — rallumé : exactement le même corps`)
    /* la casse, au zoom : tout en vw, la part d'écran ne bouge pas, le texte non plus */
    await p.locator(`${card} .doc-wreck`).click()
    assert.equal(await p.getAttribute(`${card} [style*="font-size"]`, 'data-intent'), 'statement')
    ok(await render(), body(W), `${W} — vw seul au zoom ×2 : le corps de ×1`)
    assert.match(await text(p, `${card} .badge.ko`), /pas un pixel/)
    await p.locator(`${card} .doc-wreck`).click()
    ok(await render(), body(W, 2), `${W} — réparé : le corps du zoom revient`)
    await close()
  }
})
test('1 · dans la vue, toute taille posée en ligne est un jeton ou le corps calculé, ou sa ligne (ou la précédente) dit « casse »', () => {
  ...
  src.forEach((l, i) => {
    for (const m of l.matchAll(/fontSize:\s*(`[^`]*`|"[^"]*"|[^,}]+)/g)) {
      if (/var\(--|\$\{body\}|\btoken\b/.test(m[1])) continue
...
test('2 · la gazette est fer à gauche, corps ≥ 16, interligne ≥ 1,5, capitales espacées par le style ; l’arbre décale d’une marge de carte ; le champ est au corps', async () => {
  for (const W of WIDTHS) {
    ...
    ok(await calcPx(p, '#wreck .tp-scene.field input', 'fontSize'), expected('font-size-body', W), `${W} — le champ au corps`)
    ...
    ok(await calcPx(p, '#wreck .tp-alignment', 'paddingTop'), expected('pad-1-inline', W), `${W} — la carte calée, une valeur des quatre côtés`)
    ok(await calcPx(p, '#wreck .tp-alignment', 'paddingLeft'), expected('pad-1-inline', W), `${W} — la carte calée, le côté`)
    await close()
  }
})
...
test('3 · nom orphelin, justifier, étouffer, saut de niveau, graisse, capitales, champ à 14 px, calage — chacune déclarée, rendue, réparée', async () => {
  ...
  const wreck = (i) => p.locator(`${card(i)} .doc-wreck`).click()
  /* le saut de niveau */
  await wreck(2)
  assert.deepEqual(await texts(p, `${card(2)} .gd-tree-rank`), ['h1 · Le dossier', 'h2 · Première partie', 'h4 · Un détail', 'h2 · Deuxième partie'])
  assert.ok((await p.locator(`${card(2)} .gd-tree-rank.ko`).count()) === 1 && /h3 manquant/.test(await text(p, `${card(2)} .gd-tree-note`)))
  await wreck(2); assert.equal(await p.locator(`${card(2)} .gd-tree-rank.ko`).count(), 0)
  /* la graisse */
  /* la scène d'une bande : son paragraphe, pas la parole de gauche */
  const scene = (i) => `${card(i)} .tp-scene p`
  ok(await calcPx(p, scene(3), 'fontWeight'), 400, 'graisse au repos')
  await wreck(3); ok(await calcPx(p, scene(3), 'fontWeight'), 600, 'graisse cassée'); assert.equal(await p.getAttribute(scene(3), 'data-intent'), 'statement')
  await wreck(3); ok(await calcPx(p, scene(3), 'fontWeight'), 400, 'graisse réparée')
  /* les capitales */
  assert.equal(await calc(p, scene(4), 'textTransform'), 'none')
  await wreck(4); assert.equal(await calc(p, scene(4), 'textTransform'), 'uppercase'); assert.equal(await p.getAttribute(scene(4), 'data-intent'), 'statement')
  await wreck(4); assert.equal(await calc(p, scene(4), 'textTransform'), 'none')
  /* le champ */
  await wreck(5); ok(await calcPx(p, `${card(5)} input`, 'fontSize'), 14, 'champ à 14'); assert.equal(await p.getAttribute(`${card(5)} input`, 'data-intent'), 'statement'); assert.match(await text(p, `${card(5)} .badge.ko`), /14 px/)
  await wreck(5); ok(await calcPx(p, `${card(5)} input`, 'fontSize'), expected('font-size-body', W), 'champ réparé')
  ...
  const alignment = () => p.evaluate(() => { const c = document.querySelector('#wreck .tp-alignment'), t = c.querySelector('.tp-witness'), cs = getComputedStyle(c)
    return { top: t.getBoundingClientRect().top - c.getBoundingClientRect().top, side: parseFloat(cs.paddingLeft) + parseFloat(cs.borderLeftWidth), says: c.parentElement.querySelector('.badge, .mono.muted').textContent } })
  if (knows) {
    await p.evaluate(() => document.fonts.ready); await p.waitForTimeout(100)
    const rest = await alignment(); ok(rest.top, rest.side, 'calé : le haut vaut les côtés', 1)
    assert.ok(numbers(rest.says).length >= 2 && Math.abs(numbers(rest.says)[0] - numbers(rest.says)[1]) <= 1, `calé, la page le dit : ${rest.says}`)
    await wreck(6); await p.waitForTimeout(100)
    assert.equal(await p.getAttribute('#wreck .tp-alignment', 'data-intent'), 'statement')
    const broken = await alignment(); assert.ok(broken.top > broken.side + 2, `cassé : l'air revient au-dessus (${broken.top} > ${broken.side})`)
    assert.match(await text(p, `${card(6)} .badge.ko`), /n’est plus celle qu’on voit|n'est plus celle qu'on voit/)
    await wreck(6); await p.waitForTimeout(100); const repaired = await alignment(); ok(repaired.top, repaired.side, 'réparé : le haut revient aux côtés', 1)
  } else assert.match(await text(p, `${card(6)} .badge`), /ne sait pas encore caler/)
  await close()
})
...
test('7 · la liste au corps unique : un seul corps, les graisses du moteur (titre · courant · étiquette), l’encre seconde au sous-titre ; « égaliser » ramène tout au courant et à l’encre première, puis se répare', async () => {
  ...
  const caption = await text(p, '#weight .gd-caption')
  ...
  await p.locator('#weight .button.broken').nth(0).click()
  ...
  assert.match(await text(p, '#weight .badge.ko'), /plus rien ne se distingue/)
  await p.locator('#weight .button.broken').nth(0).click()
...
test('7 · deux fonds, deux graisses : le versant sombre est un vrai thème sombre, allégé de l’écart du registre ; le curseur ne repeint que lui ; « la même graisse » est la casse, et se répare', async () => {
  ...
  await p.locator('#weight .button.broken').nth(1).click()
  ...
  assert.match(await text(p, '#weight .tp-background.black .badge.ko'), /pèse plus/)
...
test('6 · marges, espaces, coins ET tailles de texte : chaque valeur calculée est une valeur du moteur à cette largeur (déclarées exceptées) ; zéro débord ; zéro erreur', async () => {
  ...
  for (const W of WIDTHS) {
    ...
    const f = await faultsInHard(p, W, DENSITIES.comfortable)
