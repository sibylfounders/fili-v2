...
const RULES: { id: string; name: string; heading: string; statement: string; why: string; src: Src[]; div?: string }[] = [
  ...
  { id: "y8", name: "8", heading: "Les crans sont responsives — c'est le jeton qui varie, jamais l'écran",
    statement: "Chaque jeton glisse entre deux bornes, sur son axe — mais la variation vit dans la définition du jeton, une fois. Aucun écran ne redéfinit un cran.",
  ...
  { id: "y9", name: "9", heading: "La géométrie d'espacement vit en rem",
    statement: "Les jetons d'espacement s'expriment en rem. Restent en pixels, par décision explicite : le plancher de la cible du doigt, les traits d'un pixel, la largeur d'écran minimale.",
  ...
  { id: "y13", name: "13", heading: "Un seul registre, site compris",
    statement: "Les crans de page — la tête d'une section, la gouttière, le silence entre deux sections — sont la même chaîne continuée au-dessus du container. Le gabarit du site ne possède aucune valeur à lui : chaque distance qu'il consomme est un jeton dérivé.",
  ...
  { id: "y14", name: "14", heading: "Deux questions choisissent le cran",
    statement: "Est-ce un espace, une marge ou un coin ? À quelle profondeur — container, card, row, ou au plus serré ? La réponse désigne le jeton — le cran se déduit, il ne se choisit pas à l'œil.",
  ...
  { id: "y17", name: "17", heading: "La cible au doigt a un plancher",
    statement: "Un bouton, un champ, un sélecteur ont une hauteur de cible dérivée du registre ; les commandes secondaires — têtes d'outils, tiroir — prennent la cible réduite. Rien ne descend sous le plancher absolu.",
  ...
  return (
    <div className="ry-scrolls">
      <table className="table mono">
        <thead><tr><th>jeton</th><th>à la charte (px)</th><th>calculé (px, {WIDTH_MIN} → {WIDTH_MAX})</th><th>Tailwind (grille de 4)</th><th>CSS</th></tr></thead>
        <tbody>
          {names.map((n) => {
            const t = J[n];
            return (
...
const LIST: LineList[] = [
  { name: "La chaîne se dérive, elle ne s'écrit pas",
    says: "Toute distance descend d'une base unique divisée par racine de deux à chaque profondeur, et la variation vit dans la définition du jeton — jamais dans une largeur d'écran. Aucune valeur d'espacement n'est écrite à la main.",
    or: "dans le code" },
