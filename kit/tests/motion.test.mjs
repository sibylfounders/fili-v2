...
test('1 · la trace : un seul bouton déplace les deux cartes ; la bonne glisse au cran expressif sur la courbe du kit ; la mauvaise disparaît puis paraît ailleurs, en image-clé déclarée', async () => {
  ...
  assert.equal(await p.locator('#trace .demo-go').count(), 1, 'un seul bouton pour les deux côtés')
  assert.equal(await text(p, '#trace .demo-go'), 'Déplacer')
  ...
  assert.equal(await p.getAttribute('#trace .demo-side.bad', 'data-intent'), 'statement', 'la rupture est déclarée')
  await p.locator('#trace .demo-go').click(); await p.waitForTimeout(EXPRESSIVE / 8)
  ...
  assert.equal(await text(p, '#trace .demo-go'), 'Rejouer')
  assert.match(await text(p, '#trace .demo-caption'), new RegExp(`^${EXPRESSIVE} ms sur la courbe du kit, lus sur le rendu`), 'la légende dit ce qui est lu')
  await close()
})
...
test('2 · la cause : un seul geste ouvre les deux menus ; le bon grandit depuis le coin de son bouton, le mauvais depuis son centre et loin, déclaré ; le bouton de projet ouvre et ferme et dit son état', async () => {
  ...
  assert.equal(await p.getAttribute('#cause .demo-side.bad', 'data-intent'), 'statement', 'déclaré')
  ...
  await p.locator('#cause .demo-go').click(); await p.waitForTimeout(SLOW + 150)
  ...
  assert.equal(await text(p, '#cause .demo-go'), 'Fermer')
  ...
  assert.match(await text(p, '#cause .demo-caption'), new RegExp(`^${SLOW} ms, le cran du panneau · point de départ lu sur le rendu : le coin du bouton / son propre centre$`), 'la légende dit ce qui est lu')
  await close()
})
...
test('3 · le regard : une mise à jour ; côté faute, les huit cartes s’animent (déclaré) ; côté juste, une seule — celle qui a changé, étiquetée « Mis à jour » ; la suivante change de carte', async () => {
  ...
  assert.equal(await p.getAttribute('#gaze .demo-side.bad', 'data-intent'), 'statement')
  await p.locator('#gaze .demo-go').click(); await p.waitForTimeout(80)
  ...
  assert.equal(await text(p, '#gaze .demo-go'), 'Changer encore')
  await p.locator('#gaze .demo-go').click(); await p.waitForTimeout(80)
  assert.notEqual(await text(p, '#gaze .gaze-panel.is-good .gaze-card.is-target > span'), target1, 'une autre carte change')
  assert.match(await text(p, '#gaze .demo-caption'), /^8 cartes animées d'un côté, 1 de l'autre — comptées sur le rendu/, 'la légende compte ce qui bouge')
  await close()
})
...
test('4 · les quatre paires : le juste et le faux côte à côte, un seul geste joue les deux — le survol lent, le menu qui traîne, la naissance à zéro, l’ancienne règle qui coupait tout ; chaque tête de côté est lue sur le rendu', async () => {
  ...
  /* pas de geste scripté (retour d'Auteur, 9 septembre) : deux rangées identiques de petits boutons, balayées par le vrai curseur */
  assert.equal(await p.locator(`${band(1)} .demo-go`).count(), 0, 'le survol ne se joue pas au bouton')
  assert.equal(await p.locator(`${band(1)} .mv-row.slow .button`).count(), await p.locator(`${band(1)} .mv-row:not(.slow) .button`).count(), 'les deux rangées ont les mêmes boutons')
  assert.ok(await p.locator(`${band(1)} .mv-row .button`).count() >= 8, 'une rangée assez longue pour être balayée')
  assert.equal(await calcPx(p, `${band(1)} .mv-row .button`, 'minHeight'), await calcPx(p, `${band(2)} .demo-go`, 'minHeight'), 'des petits boutons : le cran compact, celui de l\'action du cadre')
  const rest = await calc(p, `${band(1)} .mv-row.slow .button`, 'backgroundColor')
  await p.locator(`${band(1)} .mv-row.slow .button`).first().hover(); await p.waitForTimeout(MOTION.durations.slow.ms * R + 60)
  assert.notEqual(await calc(p, `${band(1)} .mv-row.slow .button`, 'backgroundColor'), rest, 'survolé : le bouton répond au vrai curseur')
  ...
  /* le menu joue vraiment (retour d'Auteur, 9 septembre : « même animation des deux côtés ») : à mi-course du juste, le fautif est encore loin derrière */
  await p.locator(`${band(2)} .demo-go`).click(); await p.waitForTimeout(MOTION.durations.base.ms * R * 0.6)
  const midRight = parseFloat(await calc(p, `${band(2)} .mv-menu`, 'opacity', 1)), midWrong = parseFloat(await calc(p, `${band(2)} .mv-menu`, 'opacity', 0))
  assert.ok(midRight > midWrong + 0.15, `un geste, deux vitesses : juste ${midRight} > fautif ${midWrong}`)
  await p.waitForTimeout(EXPRESSIVE * R)
  ...
  /* rejouer depuis le départ (retour d'Auteur, 9 septembre) : la notification repart de zéro, pas d'où elle en était */
  assert.equal(await text(p, `${band(3)} .demo-go`), 'Rejouer')
  await p.locator(`${band(3)} .demo-go`).click(); await p.waitForTimeout(40)
  assert.ok(parseFloat(await calc(p, `${band(3)} .mv-toast`, 'opacity', 1)) < 0.5, 'rejouée : elle repart de presque rien')
  await p.waitForTimeout(MOTION.durations.base.ms * R + 60)
  assert.equal(parseFloat(await calc(p, `${band(3)} .mv-toast`, 'opacity', 1)), 1, 'rejouée : elle est revenue')
...
test('5 · réduit : plus un déplacement en transition ni en animation ; les fondus restent ; la bonne carte est quand même à l’arrivée, les menus s’ouvrent quand même', async () => {
  ...
  await reduced.p.locator('#gaze .demo-go').click(); await reduced.p.waitForTimeout(80)
  ...
  await reduced.p.locator('#trace .demo-go').click(); await reduced.p.waitForTimeout(80)
  ...
  await reduced.p.locator('#cause .demo-go').click(); await reduced.p.waitForTimeout(SLOW + 150)
...
test('6 · l’écriture d’Auteur : aucun mot qui décrit l’écran ou raconte le geste ; une observation sous chaque titre, une légende lue par preuve, observation / règle / réglage distingués ; pas de badge décrété, pas de pied ; par démo, un titre et un bouton ; chaque côté est nommé une fois', async () => {
  ...
  assert.equal(await p.locator('main .gdoc-sec-head .muted').count(), 5, 'une phrase d\'observation sous chaque titre — une seule')
  assert.equal(await p.locator('main .motion-demo .demo-caption').count(), 3, 'une légende par preuve, et elle porte une valeur lue')
  ...
  for (const id of ['trace', 'cause', 'gaze']) {
    assert.equal(await p.locator(`#${id} .demo-head > b`).count(), 1, `${id} : un titre`)
    assert.equal(await p.locator(`#${id} .demo-head button`).count(), 1, `${id} : un bouton`)
    assert.equal(await p.locator(`#${id} .demo-verdict`).count(), 2, `${id} : chaque côté est nommé une fois, sans badge`)
  }
...
test('7 · les tokens : la démo est une coque, la carte de tâche et la carte du tableau de bord sont des cards, le menu parle au cran ligne ; la coque suit la base de la densité ; l’affiche et les sections glissent', async () => {
  for (const W of WIDTHS) {
    ...
    ok(await calcPx(p, '#trace .demo', 'borderTopLeftRadius'), expected('r-2', W), `${W} — le cadre de la démo, coin de card`)
  ...
  for (const density of ['compact', 'airy']) {
    ...
    ok(await calcPx(p, '#trace .demo-head', 'paddingLeft'), expected('pad-2-inline', 1440, DENSITIES[density]), `${density} — la tête de démo suit la base`)
    await close()
  }
...
test('7 · le mouvement est une fondation : le rail le range avec ses sœurs ; cinq sections, trois preuves, quatre paires, huit lignes en liste, neuf lignes de code lues au moteur', async () => {
  const { p, close } = await nav.page(URL(), { width: 1440 })
