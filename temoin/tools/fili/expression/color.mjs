...
export function contrasting(background, [C, H], target, direction = 'dark') {
  for (const L of direction === 'dark' ? [0.22, 0.16, 0.1, 0.05] : [0.97, 0.985, 0.995, 1]) {
    const hex = lchToHex([L, C, H])
    if (contrast(hex, background) >= target) return hex
  }
  throw new Error(`aucune partenaire ${direction} n'atteint ${String(target)}:1 sur ${background}`)
}
export const therePlusSoft = partner
