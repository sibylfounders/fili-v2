...
export function TrackingInvoices() {
  ...
  const { launch } = useMutation('/relances')
  ...
  return (
    <div className="p-8">
        ...
        {data.map((invoice) => (
          ...
          >
            ...
            <span className="flex items-center justify-between">
              <span className="text-xs px-2 py-1 rounded bg-red-100 text-red-700">{invoice.status}</span>
              <button
                onClick={launch}
      ...
      <p className="text-xs text-gray-500 mt-4">{data.length} factures</p>
    </div>
  )
}
