...
| `/mouvement` · Mouvement | 🟢 | Fondation depuis le 7 septembre (sous Geste — arbitrage délégué). Trois preuves qui se lisent, jamais seules : la main invisible (d'abord la faute, la barre qui court après la main et l'écart peint en rouge, puis comme il faut — six étapes sous-titrées), quatre situations et une de trop (le film à la manière de `/rythme` : ce que vous faites, l'objet qui répond pour de vrai, la durée en grand et son emploi ; la cinquième est un menu qui traîne), dire ce qu'on voit (six mots, chacun joué à sa tuile au ralenti, le verdict lu dans la feuille) ; quatre paires ; huit règles en liste, neuf lignes de code | **Verrouillée le 7 septembre (deuxième fois, après la refonte à l'œil) : douze épreuves réécrites avec les preuves, trois courses vertes, course complète sur la machine d'Auteur — six pages vertes.** |
